...
namespace :build do
  ...
  task generate_indexes: :environment do
    StaticPageBuilder.new do |builder|
      builder.build_html(
        Pages::Top::IndexPageComponent.new,
        path: 'index.html'
      )

      builder.build_html(
        Pages::IndexPages::IndexTopPageComponent.new,
        path: 'index_pages/index_top.html'
      )

      builder.build_html(
        Pages::IndexPages::IndexAllPageComponent.new,
        path: 'index_pages/index_all.html'
      )

      builder.build_html(
        Pages::IndexPages::PersonAllPageComponent.new,
    ...
    StaticPageBuilder.new do |builder|
      Kana.each_sym_and_char do |id, kana|
        ...
        (1..total_page).each do |page|
          ...
          builder.build_html(
            Pages::IndexPages::WorkIndexPageComponent.new(
              id:,
              kana:,
              pagy:,
              works: current_works
    ...
    StaticPageBuilder.new do |builder|
      Kana.each_sym_and_char do |id, kana|
        ...
        (1..total_page).each do |page|
          ...
          builder.build_html(
            Pages::IndexPages::WorkInpIndexPageComponent.new(
              id:,
              kana:,
              pagy:,
              works: current_works
  ...
  task generate_soramoyou: :environment do
    StaticPageBuilder.new do |builder|
      ...
      builder.build_html(
        Pages::NewsEntries::IndexPageComponent.new(
          year: current_year
        ),
      ...
      (begin_year..current_year).each do |year|
        builder.build_html(
          Pages::NewsEntries::IndexYearPageComponent.new(year:),
  ...
  task generate_whatsnew: :environment do
    StaticPageBuilder.new do |builder|
      ...
      total_page = works.count.fdiv(item_count).ceil # 割り切れない場合は切り上げ
      (1..total_page).each do |page|
        pagy = Pagy.new(count: works.count, page:, items: item_count)
        current_works = works.offset(pagy.offset).limit(pagy.items)
        path = url.whatsnew_index_pages_path(page:, format: :html)
        builder.build_html(
          Pages::Whatsnew::IndexPageComponent.new(
            date:,
            pagy:,
            works: current_works
          ),
      ...
      (Pages::Whatsnew::IndexPageComponent::FIRST_YEAR..prev_year).each do |year|
        ...
        (1..total_page).each do |page|
          ...
          builder.build_html(
            Pages::Whatsnew::IndexYearPageComponent.new(
              year:,
              date:,
              pagy:,
              works: current_works
  ...
  task generate_person_index: :environment do
    StaticPageBuilder.new do |builder|
      Kana.each_column_key do |key|
        builder.build_html(
          Pages::People::IndexPageComponent.new(id: key),
          path: "index_pages/person_#{key}.html"
        )

        builder.build_html(
          Pages::IndexPages::PersonAllIndexPageComponent.new(id: key),
          path: "index_pages/person_all_#{key}.html"
        )

        builder.build_html(
          Pages::IndexPages::PersonInpIndexPageComponent.new(id: key),
    ...
    StaticPageBuilder.new do |builder|
      Person.find_each do |person|
        ...
        (1..total_page).each do |page|
          ...
          current_works = works.offset(pagy.offset).limit(pagy.items)
          builder.build_html(
            Pages::IndexPages::ListInpShowPageComponent.new(
              author: person,
              pagy:,
              works: current_works
            ),
